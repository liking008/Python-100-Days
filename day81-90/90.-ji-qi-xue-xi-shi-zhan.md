...
![](../.gitbook/assets/10_dataset_visualization.png)
...
2. **特征转换**：将原始数据转化为更适合模型输入的形式。常见的特征转换方法有：
   *   标准化和归一化：标准化将数据转换为均值为`0`、标准差为`1`的分布；归一化将数据缩放到特定的范围（如：$\small{\[0, 1]}$），确保各特征在同一数量级，对应的公式如下所示：
       ...
       其中，$\small{\mu}$表示数据的均值，$\small{\sigma}$表示数据的标准差，$\small{X\_{min\}}$表示数据中的最小值，$\small{X\_{max\}}$表示数据中的最大值。Scikit-learn 库`preprocessing`模块中的`StandardScaler`和`MinMaxScaler`可以实现数据的标准化和归一化。
...
![](../.gitbook/assets/10_test_web_api.png)
