...
我们可以用距离（distance）来衡量特征空间中两个实例之间的相似度，常用的距离度量包括闵氏距离、马氏距离、余弦距离、编辑距离等。闵氏距离全称闵可夫斯基距离（Minkowski Distance），对于两个 $\small{n}$ 维向量 $\small{\mathbf{x}=(x\_{1}, x\_{2}, \cdots, x\_{n})}$ 和 $\small{\mathbf{y}=(y\_{1}, y\_{2}, \cdots, y\_{n})}$ ，它们之间的距离可以定义为：
...
![](../.gitbook/assets/02_distance_measurement.jpeg)
...
![](../.gitbook/assets/02_iris_dataset.png)
...
![](../.gitbook/assets/02_scikit-learn_introduction.png)
...
6. **ROC** 和 **AUC**。
   *   **ROC**（Receiver Operating Characteristic Curve）绘制了召回率与假正例率的关系，如下图所示。
       ...
       ![](../.gitbook/assets/02_ROC_curve.png)
...
![](../.gitbook/assets/02_confusion_matrix.png)
...
![](../.gitbook/assets/02_ROC_plot.png)
...
![](../.gitbook/assets/02_k-fold_cross_validation.png)
...
![](../.gitbook/assets/02_knn_regression_plot.png)
